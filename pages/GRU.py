...
st.title("GRU Model for Stock Price Prediction")
st.write("Select a ticker to get predictions using the GRU model.")
...
if selected_ticker:
    ...
    if X is not None:
        ...
        st.subheader(f"Generating Future Predictions with GRU Model for {selected_ticker}...")
        try:
            # Load the GRU model
            gru_model_path = "models/gru_model.keras" # Assuming model is in 'models' directory
            if not os.path.exists(gru_model_path):
                gru_model_path = "ensembling/gru_model.keras" # Check ensembling directory too
            ...
            if os.path.exists(gru_model_path):
                ...
                plt.xlabel("Date")
                plt.ylabel("Price")
                plt.title(f"GRU Model: Historical and Future Price Predictions for {selected_ticker}")
                plt.legend()
                st.pyplot(fig)
                ...
                st.error(f"GRU model not found at {gru_model_path}. Please ensure 'gru_model.keras' is in the 'models' or 'ensembling' directory.")
        except Exception as e:
            st.error(f"An error occurred during prediction: {e}")
else:
    st.info("Please select a ticker to proceed with GRU prediction.")
